...
# langchain-huggingface : sentence-transformers, torch(PyTorch)같이 딸려옴

# torch (PyTorch)는 백터라이징에 필요한 행렬 계산을 해준느 엔진 : 해당 엔진위에서 임베딩, LLM모델이 구동
# sentence-transformer: transformer 기반으로 더 간단하게 청킹등의 작업을 처리해주는 편의기능

# langchain의 개념
# AI 서비스 개발을 하기 위한, 청킹, 임베딩, LLM요청, 응담, 검증등의 일괄적인 전체 프로세스를 표준화하기 위한 도구
# 만약 langchain이 없으면 로컬에서 허깅페이스버전으로 만들었을때 다른 사용 API버전으로 변경시 모든 코드구조를 일일이 변경해야함
# 이때 langchain을 쓰면 어떤 API, 모델을 쓰더라도 표준 메서드명으로 통일해서 동작되는 표준규격 어댑터 제공