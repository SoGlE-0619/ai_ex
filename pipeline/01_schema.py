...
# for name, table in tables.items():
#   # 반복되는 name(테이블명), table(테이블정보)를 이용해 build_create()함수 반복 호출
...
# 상품을 구매한 특정 고객의 정보를 찾을 때 그 상품을 구매한 고객정보 참조 테이블에서 수많은 고객정보를 매번 탐색해야됨
# 애초에 외래키 연결 시 각각의 정보에 인덱스를 붙임 (등가교환 저장할 땐 시간이 조금 더 걸리지만 찾을 땐 빠름)
