...
con.commit()
# customers 테이블에서 전체 행의 갯수를 구해서 print로 출력
# con.execute("sql문").fetchone()
# costomers_count = con.execute("SELECT COUNT(*) FROM customers").fetchone()[0]
...
# csv파일 모두 불러옴 -> 파일별로 테이블명과 들어갈 데이터분리 -> 각 데이터 별로 타입 추론 -> 테이블 생성
#  -> sql문 제작 -> sql문 실행해서 테이블생성 -> 생성된 테이블에 각 csv파일 데이터를 데이터타입에 맞게 변환해서 
# 저장 -> 생성된 데이터의 외래키 컬럼에 인덱싱 처리

# 그래서 해당 파일에 단지 데이터 확인하는 쿼리문을 날리면 그거 하나때문에 위의 무거운 프로세스가 매번 계속 실행됨
# 해결법 : 별되의 db.py를 app폴더 안쪽에 만들어서 파이프라인을 건들지 않으면서 데이터조회함수를 호출해 사용
con.close()
